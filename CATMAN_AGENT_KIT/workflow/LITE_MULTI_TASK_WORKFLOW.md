...
```text
CATMAN_AGENT_KIT/workflow/SKILL_V3_LITE.md
```
...
Полные данные не прикрепляются. Workers получают только назначенные `knowledge/retailers/Rxx.json` либо отдельные `knowledge/*.json` через GitHub.
...
```text
CATMAN_AGENT_KIT/knowledge/
```

Главное правило:

```text
orchestrator читает только RESOURCE_INDEX.json
worker читает только назначенные shards
```

Нельзя давать всем workers всю папку `knowledge/`.
